...
import samkLogo from "./assets/samk-bubble.png";
...
export default function VoiceApp() {
  ...
  useEffect(() => {
    ...
    function pickNextThinkPhrase() {
      const options = thinkPhrases.filter(
        (phrase) => phrase !== thinkPhraseRef.current,
      );
      ...
      thinkIntervalRef.current = setInterval(() => {
        ...
        if (thinkIndexRef.current >= phrase.length) {
          ...
          thinkPauseRef.current = setTimeout(
            ...
            Math.floor(5 + Math.random() * 10) * 1000,
          ); /*5-15 seconds*/
        }
      }, 100);
    }
  ...
  function checkENVvariables() {
    if (
      HA_WS_API_URL === undefined ||
      HA_ACCESS_TOKEN === undefined ||
      HA_URL === undefined
    ) {
  ...
  useEffect(() => {
    ...
    function wsConnect() {
      ...
      ws.onmessage = (event) => {
        ...
        if (msg.id === 2) {
          const entity = msg.result.find((e) => e.entity_id === ENTITY_ID);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ModelSelect />
        <button
          onClick={() => (window.location.href = "/chatbotapp")}
          style={{
      ...
      <h1
        style={{
          ...
          background:
            "linear-gradient(135deg, #00a5cd 0%, #0088aa 25%, #00c4f0 50%, #0077aa 75%, #00a5cd 100%)",
          WebkitBackgroundClip: "text",
          WebkitTextFillColor: "transparent",
          backgroundClip: "text",
          filter: "drop-shadow(0 0 8px rgba(0, 165, 205, 0.5))",
          zIndex: 1,
        }}
      >
        {t("voice.title")}
      ...
      <div style={{ zIndex: 1 }}>
        <VoiceAvatar
          loading={loading}
          isActive={
            !modelLoading && wakeListeningEnabled && haListening && !loading
          }
